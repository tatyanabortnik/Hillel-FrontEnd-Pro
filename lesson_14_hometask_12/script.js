...
const makePrototype = (arr, objectProto) => {
    let decsendant = arr 
        .map(item => {
            let decsendant = Object.create(objectProto); 
            Object.assign(decsendant, item);
...
let vegetablesFromVegetable = makePrototype(vegetables, Vegetable); 
let fruitsFromFruit = makePrototype(fruits, Fruit); 
...
document.write( renderList(vegetablesFromVegetable) );
document.write( renderList(fruitsFromFruit) );
