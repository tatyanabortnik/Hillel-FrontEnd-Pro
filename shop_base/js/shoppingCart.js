...
const renderShoppingCartHtml = async () => {
   ...
   orderSummary.addEventListener(`submit`, async (e) => {
      ...
      if (loggedInUser.shoppingCart.length) {
         loggedInUser.orders = JSON.parse(
            JSON.stringify(loggedInUser.shoppingCart)
         );
...
const renderCartItem = (item, tbody, loggedInUser, orderedItemsList) => {
   let cartItem = loggedInUser.shoppingCart.find(
...
const changeOrderTotal = async (
   ...
   if (event) {
      user.shoppingCart.forEach(
         (product) => item.id === product.id && (product.count = cartItem.count)
      );
...
const getTotalPrice = (user, orderedItemsList) => {
   let orderTotal = user.shoppingCart.reduce((acc, item) => {
      let prodPrice = getPriceWithSale(
         orderedItemsList.find((prod) => prod.id == item.id)
      );
